...
const formSchema = z.object({
	name: z.string().min(1, { message: "El nombre del color es obligatorio" }),
	value: z
		.string()
		.min(4)
		.regex(/^#/, { message: "El código debe ser un hexadecimal válido" })
		.toUpperCase(),
});
...
export const ColorForm: React.FC<ColorFormProps> = ({ initialData }) => {
	...
	const description = initialData ? "Editar el color" : "Crear un color nuevo";
	...
	const onDelete = async () => {
		try {
			...
			toast.error(
				"Asegúrate de primero eliminar todos los productos que usan este color."
			);
		} finally {
			setLoading(false);
			setOpen(false);
