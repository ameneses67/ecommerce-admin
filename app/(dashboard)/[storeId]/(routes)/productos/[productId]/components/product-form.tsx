...
const formSchema = z.object({
	...
	categoryId: z
		.string()
		.min(1, { message: "La categoría del producto es requerida." }),
	sizeId: z.string().min(1, { message: "La talla del producto es requerida." }),
	colorId: z
		.string()
		.min(1, { message: "El color del producto es requerido." }),
...
export const ProductForm: React.FC<ProductFormProps> = ({
	...
	const onSubmit = async (data: ProductFormValues) => {
		try {
			...
			if (initialData) {
				await axios.patch(
					`/api/${params.storeId}/productos/${params.productId}`,
					data
				);
			} else {
